package com.bertan.constants;

public class EndPointList {
    public static final String PREF = "/dev";
    public static final String VERSIONS = "/v1";
    public static final String USER = PREF+VERSIONS+"/user";
    public static final String KITAP = PREF+VERSIONS+"/kitap";
    public static final String YAZAR = PREF+VERSIONS+"/yazar";
    public static final String YAYINEVI = PREF+VERSIONS+"/yayinevi";
    public static final String TUR = PREF+VERSIONS+"/tur";
    public static final String FOLLOW = "/follow";
    public static final String SAVE = "/save";
    public static final String FIND_ALL = "/findall";
    public static final String FIND_BY_ID = "/findbyid";
    public static final String FIND_BY_YAZARADI = "/findbyyazaradi";
    public static final String FIND_BY_USERNAME = "/findbyusername";
    public static final String DELETE_BY_ID = "/deletebyid";
    public static final String UPDATE = "/update";

}
